...
class SLAM:
    def __init__(self, config, save_dir=None):
        ...
        self.frontend.set_deformation()
        self.warpfield_statedict = None
        ...
        self.params_gui = gui_utils.ParamsGUI(
            ...
            seq_name=self.config["Dataset"]["dataset_path"].split("/")[-1],
        )
        ...
        self.warpfield_statedict = (
            ...
            if self.frontend.warpfield_statedict is not None
            else None
        )

        if self.eval_rendering and self.warpfield_statedict is not None:
            torch.save(
                self.warpfield_statedict,
                os.path.join(self.save_dir, "warpfield_before.pth"),
            )
        ...
        if self.eval_rendering:
            ...
            while True:
                ...
                if data[0] == "sync_backend" and frontend_queue.empty():
                    ...
                    self.warpfield_statedict = (
                        clone_obj(data[4]) if data[4] is not None else None
                    )

                    break

            save_gaussians(self.gaussians, self.save_dir, "final_after_opt", final=True)
            if self.warpfield_statedict is not None:
                torch.save(
                    self.warpfield_statedict,
...
if __name__ == "__main__":
    ...
    if args.nogui is not None:
        config["Results"]["use_gui"] = args.nogui

    save_dir = None

    if args.eval:
        Log("Running 4DTAM in Evaluation Mode")
    ...
    if config["Results"]["save_results"]:
        ...
        save_dir = os.path.join(
            config["Results"]["save_dir"], path[-3], path[-2], current_datetime
        )
        ...
        run = wandb.init(
            project="4DTAM",
            name=f"Non-rigid_{tmp}_{current_datetime}",
            config=config,
