...
class FrontEnd(mp.Process):
    ...
    def tracking(self, cur_frame_idx, viewpoint):
        ...
        for tracking_itr in range(self.tracking_itr_num):
            if self.is_deform:
                prev_fid = self.cameras[cur_frame_idx - 1].fid
                time_input = (
                    torch.tensor([prev_fid], device="cuda:0")
                    .unsqueeze(0)
            ...
            if tracking_itr % 50 == 0:
                self.q_main2vis.put(
                    gui_utils.GaussianPacket(
                        ...
                        warpfield=self.warpfield_statedict,
                    )
                )
            if converged:
                break
    ...
    def sync_backend(self, data):
        ...
        self.warpfield_statedict = clone_obj(data[4]) if data[4] is not None else None

        if self.warpfield_statedict is not None:
            self.warpfield.deform.load_state_dict(self.warpfield_statedict)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.frontend_queue.empty():
                ...
                if self.config["Training"]["4dtam"]["kf_policy"] == "4dtam":
                    curr_visibility = (render_pkg["n_touched"] > 0).long()
                    create_kf = (
                        cur_frame_idx % self.config["Training"]["4dtam"]["kf_interval"]
                        == 0
                    )
                else:
                    ...
                    if self.single_thread:
                        create_kf = check_time or create_kf
